...
export const LinkButton = ({ children, className, link, href }: React.PropsWithChildren<ILinkButtonProps>) => {
	...
	return (
		<a
			className={`text-center inline-block font-normal hover:bg-blue-800 focus:outline focus:outline-4 focus:outline-sky-700/15 text-white px-1 rounded button-link ${
				className ?? ''
			}`}
			href={href}
		>
